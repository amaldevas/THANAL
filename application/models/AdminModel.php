<?php
	...
	class AdminModel extends CI_Model 
	{
		...
		public function getStaffBirthday($credentials)
		{
			$this->db->select('date_of_birth,name,id');
			$this->db->group_start();
				$this->db->where('MONTH(date_of_birth) =', $credentials['start_date_month']);
				$this->db->where('DAY(date_of_birth) >=', $credentials['start_date_day']);
					$this->db->or_group_start();
						$this->db->where('MONTH(date_of_birth) =', $credentials['end_date_month']);
						$this->db->where('DAY(date_of_birth) <=', $credentials['end_date_day']);
					$this->db->group_end();
				
			$this->db->group_end();
			$this->db->order_by('MONTH(date_of_birth) ASC,DAY(date_of_birth) ASC');
			$query=$this->db->get('staff');
			return $query->result();

		}
		public function getInmateBirthday($credentials)
		{
			$this->db->select('date_of_birth,name,id');
			$this->db->group_start();
				$this->db->where('MONTH(date_of_birth) =', $credentials['start_date_month']);
		...
		public function addInmateMedicines($credentials)
		{
			if($this->db->insert('inmate_medicines',$credentials))
			{
				return true;
			} 
			else 
		...
		public function editMedicine($id)
		{
			...
			foreach ($query->result() as $row)
			{
				...
				$credentials2['total_quantity']=$row->total_quantity;
        		...
        		$credentials2['rep_email']=$row->rep_email;
			}
			return $credentials2;
		}
